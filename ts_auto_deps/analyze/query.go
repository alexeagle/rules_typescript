...
func (a *Analyzer) findRuleProvidingImport(rules map[string]*appb.Rule, i *tazeImport) (string, error) {
	if i.knownTarget != "" {
...
// 'taze-import:' followed by a label. If such a tag is found, the label is
// returned. Otherwise, the target's own label is shortened and returned.
func redirectedLabel(target *appb.Rule) string {
	if tags := listAttribute(target, "tags"); tags != nil {
		for _, tag := range tags {
			if strings.HasPrefix(tag, "taze-import:") {
				return tag[strings.Index(tag, ":")+1:]
			}
		}
	}
...
// LoadLabels uses Blaze query to load targets associated with labels from BUILD
// files.
...
func (q *QueryBasedTargetLoader) LoadImportPaths(root string, paths []string) (map[string]*appb.Rule, error) {
	...
	for _, path := range paths {
		if trim := strings.TrimPrefix(path, "google3/"); trim != path {
			...
			t, err := q.loadFileLabel(targets[0].GetSourceFile().GetName())
			if err != nil {
				return nil, err
			}
			results[path] = t
			...
			// 'goog:' imports. That is handled by the taze binary proper.
			results[path] = nil
		} else {
...
// loadFileLabel loads the target associated with a file label.
func (q *QueryBasedTargetLoader) loadFileLabel(fileLabel string) (*appb.Rule, error) {
...
func (q *QueryBasedTargetLoader) resolveImportsInCommonLocations(results map[string]*appb.Rule, paths []string) error {
	...
	labelToPath["//third_party/javascript/angular2:platform_browser_testing"] = "@angular/platform-browser/testing/internal_test_api"
	queries = append(queries, "kind(rule, //third_party/javascript/angular2:platform_browser_testing)")
	for _, path := range paths {
	...
	for _, target := range r.GetTarget() {
		...
		if c := r.GetRuleClass(); isTypeScriptRule(c) || c == "alias" {
...
func commonModuleName(path string) string {
	if v, ok := commonModuleNames[path]; ok {
		return v
	}
	return path
}

// isTypeScriptRule checks if a class is a TypeScript rule class.
func isTypeScriptRule(class string) bool {
	for _, a := range allowedRuleClasses {
		if a == class {
			return true
		}
	}
