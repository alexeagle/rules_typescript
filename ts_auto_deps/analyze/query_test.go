...
func TestMain(m *testing.M) {
	rand.Seed(googletest.TestRandomSeed)
	if err := createWorkspaceFile(); err != nil {
		log.Fatalf("failed to create WORKSPACE file: %q", err)
	}
	os.Exit(m.Run())
}
...
func (bl *fakeTargetLoader) LoadLabels(labels []string) (map[string]*appb.Rule, error) {
...
func analyzeTargets(t *testing.T, labels []string, targets []*testTarget, files []*file) []*arpb.DependencyReport {
	...
	r, err := New(loader).Analyze(googletest.TestTmpDir, labels)
	if err != nil {
...
func TestUnnecessaryDependencies(t *testing.T) {
	...
	for _, test := range tests {
		...
		if diff := pretty.Compare(r[0].GetUnnecessaryDependency(), []string{"//r:a_lib"}); diff != "" {
	...
	r := analyzeTargets(t, []string{"a_lib"}, []*testTarget{
		...
		{"clib", `
			...
			attribute: <
				type: 5
				name: "tags"
				string_list_value: "taze-import://d:dlib"
			>`, []string{"b/c"}},
...
func createFile(path string, content ...string) error {
	if !filepath.IsAbs(path) {
		path = filepath.Join(filepath.Dir(googletest.TestTmpDir), path)
	}
...
func createWorkspaceFile() error {
	path := filepath.Join(filepath.Dir(googletest.TestSrcDir), "WORKSPACE")
