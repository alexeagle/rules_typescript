...
var (
	namespaceLookupTable = flag.String(
		...
		"/google/data/ro/teams/typescript/taze-namespaces-targets",
		"sstable mapping JavaScript namespaces to blaze targets")
)
...
func LocalUpdateFile(ctx context.Context, filePath string, contents string) error {
	return ioutil.WriteFile(filePath, []byte(contents), 0666)
}
...
func (upd *Updater) runBlazeAnalyze(buildFilePath string, bld *build.File, rules []*build.Rule) ([]*arpb.DependencyReport, error) {
	...
	out, stderr, err := upd.blazeAnalyze(buildFilePath, args)
	if err != nil {
		return nil, &AnalysisFailedError{fmt.Sprintf("running blaze analyze %s failed: %v", args, err)}
	}
	...
	if upd.removeUnusedDeclarations {
		for _, report := range reports {
			for _, fb := range report.GetFeedback() {
				...
				target := m[1]
				infof("Removing (possibly) unused ts_declaration dependency %q", target)
				...
				fmt.Fprintf(os.Stderr, "If this declaration is required, add a taze comment line to the .ts file using it:\n")
				fmt.Fprintf(os.Stderr, "    // taze: [symbol] from %s\n", target)
...
func globSources(ctx context.Context, path string, extensions []string) (srcSet, error) {
	...
	for _, extension := range extensions {
		pattern := "*." + extension
		matched, err := filepath.Glob(filepath.Join(path, pattern))
		if err != nil {
	...
	for _, p := range allSourcePaths {
		...
		// Try file.Stat as a fallback, for Google file systems.
		_, err := os.Stat(p)
		if os.IsNotExist(err) {
			infof("file.Glob returned non-existent file (dangling symlink?). Ignoring %q.", p)
			continue
		}
		if err != nil {
			return nil, fmt.Errorf("cannot stat file.Glob result %q: %v", p, err)
		}
...
func determineRuleType(path, s string) ruleType {
	...
	path = strings.TrimPrefix(strings.TrimPrefix(path, "java/com/google/testing/"), "testing/")
	if testingRegexp.MatchString(path) {
		// Directories named or below "testing" are considered test support
		// libraries, and as such should only be used in tests.
		// The top-level //testing and //j/c/g/testing directories however contain entire servers,
		// and thus are not automatically considered testonly.
		return ruleTypeTestSupport
	}
...
func updateDeps(bld *build.File, reports []*arpb.DependencyReport) (bool, error) {
	...
	changedDeps := false
	for _, report := range reports {
		infof("Applying report: %s", report.String())
		...
		for _, md := range report.MissingDependencyGroup {
			for _, d := range md.Dependency {
				...
				infof("Adding dependency on %s to %s\n", d, fullTarget)
				if addDep(bld, r, d) {
					changedDeps = true
				}
			}
		...
		for _, d := range report.UnnecessaryDependency {
			...
			infof("Removing dependency on %s from %s\n", d, fullTarget)
		...
		for _, s := range report.MissingSourceFile {
			infof("Removing missing source %s from %s\n", s, fullTarget)
...
// blazeAnalyze is used to run the underlying `blaze analyze` process.
func (upd *Updater) UpdateBUILD(ctx context.Context, path string, isRoot bool) (bool, error) {
	...
	if _, err := os.Stat(buildFilePath); err != nil && os.IsNotExist(err) {
		...
		if err != nil {
			infof("Could not find any ancestor BUILD for %q, continuing with a new BUILD file",
				buildFilePath)
		} else if !hasTazeEnabled(ancestor) {
			fmt.Printf("taze disabled below %q\n", ancestor.Path)
			return false, nil
		} else {
			infof("BUILD file missing and taze is enabled below %q. Creating new BUILD file.",
				ancestor.Path)
		}
	}
	...
	infof("Globbing TS sources in %s", path)
	...
	infof("Running blaze analyze (first pass)")
	...
	infof("Updating ts_config")
	if updateTSConfig(bld, isRoot) {
		changedSecondPass = true
	}
	if updateTSDevelopmentSources(bld, isRoot) {
		changedSecondPass = true
	}
	infof("Setting library rule kinds")
	...
	if tr := getRule(bld, "ts_library", ruleTypeTest); tr != nil {
		infof("Registering test rule in closest ts_config & ts_development_sources")
...
// hasTazeEnabled checks if the BUILD file should be managed using taze.
...
func QueryBasedBlazeAnalyze(buildFilePath string, args []string) ([]byte, []byte, error) {
...
func (upd *Updater) registerTestRule(ctx context.Context, bld *build.File, ruleKind string, rt ruleType, g3root, target string) error {
	...
	if r != nil {
		if addDep(bld, r, target) {
			fmt.Printf("Registered test %s in %s\n", target, absoluteBlazeTarget(bld, r.Name()))
	...
	for parentDir != "." && parentDir != "/" {
		...
		if _, err := os.Stat(buildFile); err == nil {
			...
			infof("taze disabled on %q", buildFile)
			// Continue below.
		}
...
func updateTSConfig(bld *build.File, add bool) bool {
	if !add && len(buildRules(bld, "ts_config")) == 0 {
		return false
	}
	changed := false
	r := getRule(bld, "ts_config", ruleTypeAny)
	if r == nil {
		if !add {
			return false
		}
...
func updateTSDevelopmentSources(bld *build.File, add bool) bool {
	changed := false
	targets := allTSRules(bld)

	if add {
		ruleName := "devsrcs"
		if getRule(bld, "ts_development_sources", ruleTypeRegular) == nil {
			r := getOrCreateRule(bld, ruleName, "ts_development_sources", ruleTypeRegular)
			r.SetAttr("deps", &build.ListExpr{})
			changed = true
		}
		if getRule(bld, "ts_development_sources", ruleTypeTest) == nil {
	...
	for _, t := range targets {
		...
		depName := ":" + t.Name()
		if targetRegisteredInRule(bld, "ts_development_sources", rt, depName) {
			continue
		}
		r := getRule(bld, "ts_development_sources", rt)
		if r == nil {
			continue // No devsources rule that needs updating.
		}
		if addDep(bld, r, depName) {
			changed = true
		}
	}
	return changed
...
func setLibraryRuleKinds(ctx context.Context, buildFilePath string, bld *build.File) (bool, error) {
	...
	for _, r := range buildRules(bld, "ts_library") {
		...
		if isNgModule && !shouldBeNgModule {
			infof("Changing rule %s to ts_library()", r.AttrString("name"))
			r.Call.X.(*build.Ident).Name = "ts_library"
			r.DelAttr("assets")
			changed = true
		} else if !isNgModule && shouldBeNgModule {
			infof("Changing rule %s to ng_module()", r.AttrString("name"))
...
func updateWebAssets(ctx context.Context, buildFilePath string, bld *build.File) (bool, error) {
	...
	infof("Found asset files in %s: %v", absolutePkgPath, assetFiles)
...
/* TODO(jdhamlik): Put back in updater.go in google3 */
