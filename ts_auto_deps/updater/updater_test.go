...
func mktmp(fn string, content []byte) (string, error) {
	p := fn
	if !filepath.IsAbs(p) {
		p = filepath.Join(googletest.TestTmpDir, fn)
	}
...
`))
	...
	bld, err := readBUILD(context.Background(), filepath.Join(googletest.TestTmpDir, "google3"), p)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGlobSources(t *testing.T) {
	...
	if err := os.Symlink("../blaze-bin/symlink.d.ts", filepath.Join(googletest.TestTmpDir, "symlink.d.ts")); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("whatever", filepath.Join(googletest.TestTmpDir, "whatever.d.ts")); err != nil {
		t.Fatal(err)
	}
	srcs, err := globSources(context.Background(), googletest.TestTmpDir, []string{"ts", "tsx"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDetermineRuleType(t *testing.T) {
	tests := []struct {
		...
		{"java/com/google/myapp/testing/BUILD", "foo.ts", ruleTypeTestSupport},
		{"java/com/google/myapp/testing/BUILD", "foo_test.ts", ruleTypeTest},
...
func TestRegisterTestRule(t *testing.T) {
	...
	_, err = mktmp("google3/foo/bar/BUILD", []byte(`ts_library(name = "bar_test")`))
	if err != nil {
		t.Fatal(err)
	}
	_, err = mktmp("google3/foo/baz/BUILD", []byte(`ts_library(name = "baz_test")`))
	if err != nil {
		t.Fatal(err)
	}

	bld, err := build.ParseBuild("foo/bar/BUILD", []byte(`ts_library(name = "bar_test")`))
	if err != nil {
		t.Fatal(err)
	}
	...
	updater := New(false, false, LocalBlazeAnalyze, testUpdateFile)
	err = updater.registerTestRule(ctx, bld, "ts_config", ruleTypeAny, g3root, "//foo/bar:bar_test")
	if err != nil {
		t.Fatal(err)
	}
	if updatedFile != "" {
	...
	bld, err = build.ParseBuild("foo/baz/BUILD", []byte(`ts_library(name = "baz_test")`))
	if err != nil {
		t.Fatal(err)
	}
	err = updater.registerTestRule(ctx, bld, "ts_config", ruleTypeAny, g3root, "//foo/baz:baz_test")
	if err != nil {
		t.Fatal(err)
	}
